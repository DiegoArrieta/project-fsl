...
/**
 * GET /api/proveedores/[id]/personas-contacto/[personaId]
 * PUT /api/proveedores/[id]/personas-contacto/[personaId]
 * DELETE /api/proveedores/[id]/personas-contacto/[personaId]
 */
export async function GET(
  ...
  try {
    ...
    const row = await prisma.contactoPersona.findFirst({
      where: { id: personaId, proveedorId: id },
    })
    ...
    const updated = await prisma.$transaction(async (tx) => {
      if (parsed.esPrincipal) {
        await tx.contactoPersona.updateMany({
          where: { proveedorId: id, esPrincipal: true, id: { not: personaId } },
