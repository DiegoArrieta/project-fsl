...
3. **Job `deploy`** (entorno **`production`** en GitHub):
   - Copia `deploy/prod/docker-compose.yml` al VPS (`appleboy/scp-action`); mismo orden que LDS Planner: primero SCP, luego SSH con `sudo mkdir`/`chown` del directorio de deploy.
   - Por SSH: escribe `.env`, `docker login ghcr.io`, `docker compose pull`, levanta **Postgres**, espera healthy, levanta **app**, `prisma:generate`, `prisma:migrate:deploy`, **restart** app, `docker image prune -f`, `docker compose ps`.
...
**Requisito en el VPS:** igual que en LDS Planner: **Docker Compose v2** (`docker compose` como subcomando de `docker`) y permisos para que `VPS_USER` ejecute `sudo mkdir`/`chown` bajo `/opt/forestal-santa-lucia` y use Docker (grupo `docker` o equivalente).
